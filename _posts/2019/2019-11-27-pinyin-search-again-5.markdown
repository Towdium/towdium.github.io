...
你可能有点疑惑，如果我们只缓存短串的结果，那么对于长串是不是就毫无帮助呢？当然不是，否则这个缓存结构的存在就毫无意义。对于文本匹配我们有这一结论：如果 a 是 b 的子串，b 是 c 的子串，则 a 是 c 的子串。对于拼音搜索，我对算法进行了轻度的魔改，使得它满足这一条件：如果 a 是 b 的前缀，拼音 b 匹配文本 c，则拼音 a 匹配文本 c。这是我在之前的文章里反复讨论过的思路：由于这个关系，只要 a 是 b 的前缀，则 b 的搜索结果的集合必然是 a 的结果的子集。基于这个原理，当我们搜索一个长串时，只需要寻找它前缀所对应结果的集合，再对这个集合内的条目（而不是总集）进行遍历搜索即可。
...
测试使用两组数据。第一组提取自 Enigmatica 模组包，可以近似模拟极端条件下的 Minecraft 环境，文本文件 902KB，共 37k 词条，424k 字符。第二组提取自腾讯 AILab 的汉语短语 [语料库][1]，文本文件 14.4MB，共 1M 词条，4.81M 字符。测试在 i7-7700HQ 上进行，仅使用单核运算。对于每组数据，我们分别测量了用于前缀匹配与用于包含匹配的两组数据。
...
说到索引构造的性能问题，这个项目中使用的后缀数实现实际上是非常粗糙的，在构造环节，每个条目的时间复杂度线性于条目长度的平方。我们目前的使用场景主要是中文短语，所以这一复杂度并不会带来过于严重的后果，但是如果我们要用它进行长串的匹配就需要格外小心了。这个问题无疑是本项目重要的发展方向。

除此之外，如前文所述，输入法通常需要执行全文匹配，而这一逻辑尚不支持。从理论上而言，全文匹配的核心内容已经完全就绪，但是尚未被实现出来。如果我有充足的时间投入到开发中，这一部分的内容自然是计划中的。一旦全文匹配开发完成，我们就可以真正使用它来开发非常基础的输入法了。
